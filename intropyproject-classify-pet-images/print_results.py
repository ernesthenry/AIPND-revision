...
def print_results(results_dic, results_stats_dic, model, 
    ...
    # IF print_incorrect_dogs == True AND there were misclassified dogs, print them
    if (print_incorrect_dogs and 
        ((results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs']) != results_stats_dic['n_images'])):
        print("\nINCORRECT Dog/NOT Dog Assignments:")
        for key in results_dic:
            # Pet Image Label is-a-dog (idx 3) != Classifier classifies as-a-dog (idx 4)
            if sum(results_dic[key][3:]) == 1:
                print("Real: {:>26}   Classifier: {:>30}".format(results_dic[key][0], results_dic[key][1]))

    # IF print_incorrect_breed == True AND there were misclassified breeds, print them
    if (print_incorrect_breed and 
        (results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed'])):
        print("\nINCORRECT Dog Breed Assignment:")
        for key in results_dic:
            # Pet Image Label is-a-dog and Classifier is-a-dog, but labels do not match (idx 2 == 0)
